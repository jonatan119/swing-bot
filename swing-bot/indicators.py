...
def trend_alignment_ok(row):
    """
    Price > 50 EMA, Price > 150 SMA, Price > 200 SMA - checked
    INDEPENDENTLY, with a tolerance buffer (config.TREND_ALIGNMENT_TOLERANCE_PCT).

    NOTE: this no longer requires the moving averages themselves to be
    perfectly stacked (50 EMA > 150 SMA > 200 SMA). That stricter
    requirement was removed deliberately - trading above all three key
    levels is the actual intent, without demanding a flawless, textbook
    trend history on top of it.
    """
    ...
    return (
        ...
        and row["close"] > row["sma150"] * tol
        and row["close"] > row["sma200"] * tol
    )
