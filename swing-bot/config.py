...
MIN_AVG_VOLUME = 500_000           # shares/day - checked against BOTH the 20-day
                                    ...
                                    # evaluate_ticker in scanner.py) - this is
                                    # the REAL enforced threshold, using
                                    # stable EOD data.
# A much looser floor used only at the screener API call itself - see the
# note in data_provider.get_screener_universe for why the full 500K figure
# isn't used there (the screener's volume field appears to reflect today's
# still-accumulating intraday volume, causing wildly inconsistent universe
# sizes depending on what time of day the scan happens to run).
SCREENER_VOLUME_PRE_FILTER = 50_000
...
VOLUME_SPIKE_MIN = 0.85            # loosened from 110% to 85% of average per explicit request
...
MIN_DAY_GAIN_PCT = -1.5   # widened to allow slight pullback (red) days through
MAX_DAY_GAIN_PCT = 7.0   # widened further per explicit request
...
# should sum to 1.0.
SCORE_WEIGHTS = {
    "trend": 0.15,                  # renamed conceptually to "Trend Proximity" - see scoring.py
    "macd": 0.15,
    "volume": 0.20,
    "relative_strength": 0.25,
    "pattern_confirmation": 0.20,
    "analyst_rating": 0.05,
}
# How far above the 50 EMA (in %) a stock can be before it's considered
# fully "extended" for scoring purposes (score -> 0 for this factor). Used
# by the Trend Proximity score - a tight, healthy base close to the 50 EMA
# scores HIGHER than a stock that's already run far above it, since a
# parabolic/overextended move is a worse risk/reward entry than a tight one,
# even though both technically clear the "price > 50 EMA" hard gate.
SCORE_MAX_TREND_EXTENSION_PCT = 15.0
