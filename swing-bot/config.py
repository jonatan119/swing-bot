...
ATR_WINDOW = 14
ATR_STOP_MULTIPLIER = 1.5      # fallback stop distance when no clear support level exists
...
# Hard FLOOR on stop-loss distance, expressed as a multiple of ATR. A
# support level can sit very close to entry (e.g. right at a just-broken
# resistance level) - without this floor, the stop ends up tighter than the
# stock's own normal daily noise, guaranteeing a fast, meaningless stop-out
# regardless of whether the trade thesis was right. If the support-based
# stop is tighter than this floor, it gets widened out to the floor
# distance instead. If even the floor distance itself exceeds
# MAX_STOP_LOSS_PCT (i.e. the stock is simply too volatile for the risk
# budget), the setup is rejected outright rather than forcing an unsafe
# tight stop just to keep the trade alive.
...
MAX_ENTRY_STALENESS_PCT = 2.0   # reject if live price > entry_price * (1 + this%)
