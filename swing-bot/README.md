...
- Price action: a bullish candlestick pattern OR a chart-pattern breakout
  (Cup and Handle, Ascending Triangle, Bull Flag, Double Bottom, Inverse
  Head and Shoulders, VCP).
...
- **Stop-loss** = nearest swing support level (with a small buffer below it),
  or an ATR-based fallback if no clear support exists — whichever is
  *tighter* also respects a hard `MAX_STOP_LOSS_PCT` cap (default 5%).
...
- 14-day alert cooldown per symbol (persisted across runs via
  `state/alerted_symbols.json`, auto-committed by the GitHub Actions workflow).
