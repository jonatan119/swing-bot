...
def evaluate_ticker(symbol, exchange, spy_df, rejection_counts):
    ...
    # NOTE: MACD is no longer a hard gate. It's purely a SCORING factor now
    # (see scoring.py's macd_score, weighted at 15%) - a stock isn't
    # rejected outright just because MACD didn't cross within the exact
    # last N days. A bullish MACD still earns more points; a bearish/flat
    # MACD earns fewer, but doesn't disqualify the setup on its own.

    # Price-action trigger: a single-bar candlestick pattern, a multi-bar
    # chart pattern breakout, a POC volume test, or an ascending trendline
    # bounce all qualify.
    ...
    poc_test = entry_strategy.poc_volume_test(df)
    trendline_valid, _ = entry_strategy.trendline_bounce_test(df)

    pattern_names = (
        ([candle_pattern] if candle_pattern else [])
        + [name for name, _, _ in chart_matches]
        + (["POC Volume Test"] if poc_test else [])
        + (["Uptrend Trendline Bounce"] if trendline_valid else [])
    )
    ...
    if config.CHECK_LIVE_PRICE_STALENESS:
        ...
        if live_price is not None:
            ...
            # Symmetric check: has price already DROPPED since the signal
            # was generated? A "POC Support Hold" or "Support Bounce" entry
            # means "price is holding near this support level" - if live
            # price has since fallen meaningfully BELOW the calculated
            # entry, that thesis may already be broken (or at minimum, the
            # entry price shown is stale and no longer reflects where the
            # market actually is - buying "at" a price above where the
            # stock is currently trading makes no sense).
            if staleness_pct < -config.MAX_NEGATIVE_STALENESS_PCT:
                return reject("price_already_dropped_below_entry")

            # If this was a "Resistance Breakout" entry but live price has
            # already moved meaningfully beyond that breakout level (just
            # not enough to trigger the hard reject above), the breakout
            # has already happened and run - it's stale to present as a
            # fresh breakout entry. Reclassify: the old resistance level is
            # now the relevant SUPPORT to watch for a pullback/retest,
            # rather than suggesting to buy immediately at a price already
            # well below the live market.
            if (
                entry_info["entry_type"] == "Resistance Breakout"
                and staleness_pct > config.SUPPORT_RETEST_RECLASSIFY_PCT
            ):
                entry_info["entry_type"] = "Support Retest (former resistance)"
    ...
    return {
        ...
        "atr_multiplier_used": trade_plan["atr_multiplier_used"],
        "rating": rating,
        "ema50": float(row["ema50"]),
...
def run_scan():
    ...
    # ---- Send alerts ----
    # State is saved after EVERY alert (not just once at the end), and each
    # alert is wrapped individually so one bad alert can't crash the whole
    # run and silently lose cooldown tracking for every alert already sent
    # in this same run - exactly what happened with an earlier bug.
    matched = []
    for setup in final_alerts:
        ...
        try:
            ...
            print(f"  ERROR sending alert for {symbol}: {exc}", file=sys.stderr)
            continue
