...
def detect_ascending_triangle(df, lookback=60, tolerance=0.03):
    ...
    # Require the base to represent a GENUINE pause, not just minor natural
    # pullbacks during an ordinary smooth climb. Without this, any steadily
    # trending stock will trivially satisfy "3 highs within 3%" + "rising
    # lows" purely from normal day-to-day noise, even deep into an already-
    # extended move (exactly what happened with the ROKU alert - the real
    # breakout had already occurred and run ~33% before this "triangle" was
    # detected on top of it).
    base_range_pct = (resistance_level - base_low) / resistance_level
    if base_range_pct < config.MIN_BASE_RANGE_PCT / 100:
        return None
...
def detect_double_bottom(df, lookback=90, tolerance=0.05, min_separation=10):
    ...
    # Require genuine DEPTH, not just two similarly-priced points. Without
    # this, any two minor pullback lows in an ongoing uptrend (a normal
    # staircase pattern) can falsely match as a "double bottom", using
    # whatever small local high sits between them as the neckline - even
    # though that's just noise, not a real prior resistance level.
    depth1 = (neckline - low1) / neckline   # how far below the neckline low1 sits, as a fraction
    depth2 = (neckline - low2) / neckline
    if depth1 < config.MIN_PATTERN_DEPTH_PCT / 100 or depth2 < config.MIN_PATTERN_DEPTH_PCT / 100:
        return None
...
def detect_inverse_head_shoulders(df, lookback=90, shoulder_tolerance=0.08):
    ...
    # Same fix as double bottom: require the head to be MEANINGFULLY below
    # the neckline, not just technically lower than two nearby shoulders.
    # Without this, minor noise in an ongoing uptrend can falsely match.
    head_depth = (neckline - head) / neckline
    if head_depth < config.MIN_PATTERN_DEPTH_PCT / 100:
        return None
...
def detect_vcp(df, lookback=100, min_contractions=2):
    ...
    # Same fix as ascending triangle: require the FIRST (largest) pullback
    # in the contraction sequence to have genuine depth, not just be a
    # slightly-bigger sliver of noise than the next one. Without this, tiny
    # ~1% wiggles that happen to shrink to ~0.5% would trivially count as a
    # "VCP" even during an ordinary smooth climb with no real base at all.
    if depths[0] < config.MIN_BASE_RANGE_PCT / 100:
        return None
