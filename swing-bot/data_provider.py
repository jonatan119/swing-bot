...
def get_screener_universe():
    ...
    params = {
        ...
        # NOTE: intentionally NOT using the full MIN_AVG_VOLUME threshold
        # here. This screener endpoint's volume field appears to reflect
        # TODAY's still-accumulating intraday volume, not a stable daily
        # figure - meaning a scan running early in the trading day would see
        # a dramatically smaller universe than one running later, purely
        # from timing (observed: universe size swinging from ~600 to ~1750
        # across different run times with identical config). Using a much
        # looser floor here just filters out genuinely dead/illiquid names
        # for API efficiency - the REAL 500K threshold is still fully
        # enforced per-ticker later, against stable EOD data (see
        # evaluate_ticker's current_volume_too_low / avg_volume_too_low
        # checks in scanner.py), so nothing under-volume can slip through
        # to an actual alert.
        "volumeMoreThan": config.SCREENER_VOLUME_PRE_FILTER,
        "exchange": ",".join(config.EXCHANGES),
        "country": config.SCREENER_COUNTRY,
        "isActivelyTrading": "true",
        "isEtf": "false",    # exclude ETFs (e.g. SSO, SPY) - only individual company stocks
        "isFund": "false",   # exclude mutual funds / closed-end funds too
    ...
    if not isinstance(data, list):
        ...
        return []
    # Defensive secondary filter: even with isEtf/isFund=false requested
    # above, double-check any isEtf/isFund flag present in the response
    # itself, in case the API parameter doesn't catch every edge case (e.g.
    # leveraged/inverse ETFs like SSO sometimes get miscategorized).
    return [
        {"symbol": row["symbol"], "exchange": row.get("exchangeShortName") or row.get("exchange") or ""}
        for row in data
        if "symbol" in row and not row.get("isEtf") and not row.get("isFund")
    ]
