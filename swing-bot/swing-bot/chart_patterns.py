...
Each detector returns the pattern's breakout level (float) on a match, or
None otherwise - the level itself (not just True/False) is what lets the
entry-strategy module set a real resistance-breakout entry price instead of
just using today's close.
...
def detect_cup_and_handle(df, lookback=130, handle_max_len=15):
    ...
    Returns the rim price (breakout level) on a match, else None.
    """
...
def detect_ascending_triangle(df, lookback=60, tolerance=0.02):
    """Returns the resistance level (breakout level) on a match, else None."""
    window = df.tail(lookback).reset_index(drop=True)
    if len(window) < 20:
        return None
    ...
    if latest_close > resistance_level * (1 + tolerance / 2) and window["volume"].iloc[-1] > avg_vol:
        return float(resistance_level)
    return None


# ----------------------------------------------------------------------- Bull Flag
def detect_bull_flag(df, pole_window=10, flag_window=15, min_pole_gain=0.15):
    """Returns the flag's breakout level on a match, else None."""
...
def detect_double_bottom(df, lookback=90, tolerance=0.03, min_separation=10):
    """Returns the neckline (breakout level) on a match, else None."""
    window = df.tail(lookback).reset_index(drop=True)
    if len(window) < 30:
        return None
...
def detect_vcp(df, lookback=100, min_contractions=2):
    ...
    expanding volume. Returns the pivot high (breakout level) on a match.
    """
    window = df.tail(lookback).reset_index(drop=True)
    if len(window) < 40:
        return None
...
def detect_chart_patterns(df):
    """
    Returns a list of (pattern_name, breakout_level) tuples for every
    enabled pattern that matched (usually 0 or 1 entries).
    """
    matches = []
