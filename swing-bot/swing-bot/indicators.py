...
def risk_management_levels(entry_price, atr_value):
    ...
    stop_pct (% below entry), target_pct (% above entry), atr_pct.

    Stop-loss uses whichever is TIGHTER (closer to entry) of:
      - the ATR-based stop (entry - ATR_STOP_MULTIPLIER * ATR)
      - a hard % cap (MAX_STOP_LOSS_PCT) on the entry price
    This prevents a wide ATR on a volatile stock from producing an oversized
    stop (e.g. a stock with a 6% ATR would otherwise get a ~9% stop at 1.5x -
    the cap keeps risk sane regardless of how volatile the name is).
    """
    atr_stop = entry_price - config.ATR_STOP_MULTIPLIER * atr_value
    pct_cap_stop = entry_price * (1 - config.MAX_STOP_LOSS_PCT / 100)
    stop_loss = max(atr_stop, pct_cap_stop)  # tighter of the two (closer to entry)
    ...
    return {
        ...
        "stop_capped": pct_cap_stop > atr_stop,  # True if the % cap kicked in over the raw ATR stop
    }
