...
def send_summary(num_scanned, num_matched, tickers_matched, note=None, rejection_counts=None):
    ...
    # Surface WHERE candidates fell out directly in Discord, so this is
    # visible without ever needing to open the GitHub Actions log.
    if rejection_counts:
        top_reasons = sorted(rejection_counts.items(), key=lambda kv: -kv[1])[:6]
        breakdown = "\n".join(f"  {reason}: {count}" for reason, count in top_reasons)
        content += f"\n\n**Why tickers were rejected (top reasons):**\n```\n{breakdown}\n```"
