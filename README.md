...
- Price action: a bullish candlestick pattern, a chart-pattern breakout
  ...
  Head and Shoulders, VCP), **a POC Volume Test** — price holding at the
  volume-profile Point of Control (the single most heavily-traded price
  level recently) with today's volume elevated (`VOLUME_SPIKE_MIN`) — or an
  **Uptrend Trendline Bounce** — a genuine ascending trendline fit
  (least-squares) through the recent swing lows, with price currently
  holding/bouncing off that line and volume confirming it. Both the POC
  test and the trendline bounce are standalone triggers in their own
  right, not just fallback entry references — either can qualify a setup
  even with no candlestick or chart pattern present.
